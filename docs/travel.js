...
let destinations = [
    ...
    { code: 20, name: "Vilenuse", price: 320 },
...
add_order('dany', '828', 3, 31, destinations);
...
function add_order(name, zeut, pasengers, travel_id, destinations) {
    ...
    try {
        ...
        ordernung.push({
            zeut: zeut, name: name, id_travel: travel_id, pasengers: pasengers,
...
function find() {
    ...
    try {
        ...
        ordernung.forEach(ordItem => {
            if (ordItem.name == user_find || ordItem.name == uperCaseFirst(user_find)) {
...
function find_destination() {
    ...
    try {
        let findHTML = document.querySelector('.select_destination tbody');
        let template_find = `<tr><td>#ZEUT</td><td>#NAME</td>
        <td>#ID-TRAVEL</td><td>#DESTINATION</td><td>#PASSENGERS</td><td>$#TOTAL-PRICE</td></tr>`
        ...
        ordernung.forEach(ordItem => {
            console.log("toLocaleUpperCase", user_find.toLocaleUpperCase(),
                "toLocaleLowerCase", user_find.toLocaleLowerCase(),
                "toLowerCase()", user_find.toLowerCase()


            )
            if (ordItem.destination == user_find || ordItem.destination == uperCaseFirst(user_find)) {
